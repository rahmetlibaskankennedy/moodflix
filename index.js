...
builder.defineStreamHandler(async ({ type, id }) => {
  ...
  try {
    ...
    console.log(`🎥 Stream isteği → tmdb:${tmdbId} | ${mediatype} | S${season}E${episode}`);

    const streams = await sinewix.getStreams(tmdbId, mediatype, season, episode);
    console.log(`✅ Sinewix → ${streams.length} stream bulundu`);
    return { streams };
  } catch (err) {
    console.error(`❌ Stream hata [${id}]:`, err.message);
    return { streams: [] };
  }
});
